...
public class SortedDoublyLinkedList<T extends Comparable<T>> extends CirDoublyLinkedList<T>{
    ...
    public void insert(T x){
        if (x == null)
            return;                    //不能插入空对象
        if (this.head.pred != head && this.head.pred.data.compareTo(x) < 0){//最大值插入在头结点之前，即插入在链尾，O(1)
...
//            System.out.println(this.toString()+"，p="+p.data.toString());
    ...
    //深拷贝构造方法，由循环双链表list构造排序循环双链表，直接选择排序，删除再插入算法
/*    public SortedDoublyLinkedList(CirDoublyLinkedList<T> list)
    ...
    //深拷贝构造方法，由循环双链表list构造排序循环双链表，直接选择排序，交换元素算法
    ...
    //归并两条排序循环双链表，将list中所有结点归并到当前排序循环双链表中，合并后设置list为空
